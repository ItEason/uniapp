  ...
  <view class="viewport">
    ...
    <view class="login">
      ...
      <view class="extra">
        ...
        <view class="options">
          <!-- 通用模拟登录 -->
          <button @tap.prevent="onGetphoneNumberSimple">
...
const onGetphoneNumber: UniHelper.ButtonOnGetphonenumber = async (e) => {
  let data: loginParam = { code, encryptedData: e.detail.encryptedData!, iv: e.detail.iv }
  const result = await postLoginWxMinAPI(data);
  loginSuccess(result.result)
}

//获取用户手机号码(企业中写法)
const onGetphoneNumberSimple = async () => {
  const result = await postLoginWxMinSimpleAPI({ phoneNumber: '13232137912' });
  loginSuccess(result.result)
}
